...
| Account under a manager | Handled automatically and **per tenant**: the connector discovers each connecting user's own hierarchy at OAuth time and stores the right `login-customer-id` per connection (`google_accounts.login_customer_id`). There is no global manager id — your MCC is never imposed on other users' accounts. |
