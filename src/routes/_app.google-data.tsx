...
export const Route = createFileRoute("/_app/google-data")({
  component: GoogleData,
});
...
function GoogleData() {
  ...
  const money = useMemo(() => {
    const code = googleAccount?.currency || "USD";
    try {
  ...
  }, [googleAccount?.currency]);

  const totals = useMemo(() => {
    const spend = googleMonthly.reduce((s, m) => s + m.spend, 0);
    const conversions = googleMonthly.reduce((s, m) => s + m.conversions, 0);
    const conversionValue = googleMonthly.reduce(
  ...
  // the Google feed to keep the figure consistent with the Exit Score.
  const metrics = useMemo(
    () =>
      computeMetrics({
        ...
        google: { monthly: googleMonthly, campaigns: googleCampaigns },
      }),
    [
      store,
      orders,
  ...
  const totalCampaignSpend = useMemo(
    () => googleCampaigns.reduce((s, c) => s + c.spend, 0),
    [googleCampaigns],
  );
  ...
  useEffect(() => {
    if (autoTried.current || loading || !canResyncGoogle) return;
    autoTried.current = true;
    const stale =
      !googleLastSyncedAt ||
      Date.now() - new Date(googleLastSyncedAt).getTime() > STALE_MS;
    if (stale) runResync();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, canResyncGoogle, googleLastSyncedAt]);
    ...
    return (
      <>
        <PageHeader
          ...
          subtitle="We build your marketing efficiency, ROAS and CAC on high-intent channels directly from your ad data. Connect Google Ads to get started."
        />
        <div className="card-light p-10 rounded-lg text-center max-w-xl mx-auto">
          ...
          <p className="mt-3 text-[15px] text-[var(--text-secondary)]">
            Once your Google Ads account is connected, we pull your spend, ROAS
          ...
          >
            Connect Google Ads <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </>
    );
  ...
  const months = googleMonthly.map((m) => m.month).sort();
  const windowLabel =
  ...
  return (
    <>
      <PageHeader
        title="Google Ads Data"
        subtitle="Everything we pulled from your Google Ads account. Reports are computed from this data on demand."
        right={
          <div className="flex flex-col items-end gap-2">
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={runResync}
                disabled={syncing || !canResyncGoogle}
            ...
            <span className="text-[11px] text-[var(--text-muted)]">
              {googleLastSyncedAt
                ? `Last synced ${new Date(googleLastSyncedAt).toLocaleString("en-GB")}`
                : "Not synced yet"}
            </span>
          </div>
        }
      ...
      <div className="card-light p-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <Field label="Account" value={googleAccount?.name || "—"} />
        <Field
          label="Customer ID"
          value={googleAccount?.customerId || "—"}
          mono
        />
        <Field label="Currency" value={googleAccount?.currency || "—"} />
        <Field label="Timezone" value={googleAccount?.timezone || "—"} />
        <Field label="Status" value={googleAccount?.accountStatus || "—"} />
        <Field label="Reporting window" value={windowLabel} />
        <Field label="Months of data" value={String(googleMonthly.length)} />
        <Field label="Campaigns" value={String(googleCampaigns.length)} />
      </div>
      ...
      <div className="mt-10 flex gap-1 border-b border-[var(--border-warm)]">
        {(["monthly", "campaigns"] as Tab[]).map((t) => (
          ...
          >
            {t} (
            {t === "monthly" ? googleMonthly.length : googleCampaigns.length})
          </button>
        ))}
      </div>
      ...
      <div className="mt-5">
        ...
        {tab === "campaigns" && (
          <DataTable
            ...
            note={
              googleCampaigns.length > 500
                ? `Showing first 500 of ${googleCampaigns.length}.`
                : undefined
            }
          />
        )}
